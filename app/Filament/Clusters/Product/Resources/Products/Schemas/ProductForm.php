...
class ProductForm
{
    public static function configure(Schema $schema): Schema
    {
        return $schema
            ->schema([
                self::getProductInfoSection(),
                self::getPriceAndTaxSection(),
                self::getDimensionAndQuantitySection(),
                self::getAttributesSection(),
                self::getAssignmentsSection(),
                self::getStatusSection(),
            ]);
    }

    protected static function getProductInfoSection(): Section
    {
        return Section::make(__('filament.product.product_info'))
    ...
    protected static function getPriceAndTaxSection(): Section
    {
        return Section::make(__('filament.product.price_and_tax'))
    ...
    protected static function getDimensionAndQuantitySection(): Section
    {
        return Section::make(__('filament.product.dimension_and_quantity'))
    ...
    protected static function getAttributesSection(): Section
    {
        return Section::make(__('filament.product.attributes'))
    ...
    protected static function getAssignmentsSection(): Section
    {
        return Section::make(__('filament.product.assignments'))
            ->description(__('filament.product.assignments_info'))
            ->collapsible()
            ->collapsed()
            ->schema([
                self::createTeamSelect(TeamType::SALE),
                self::createUserSelect(TeamType::SALE),

                self::createTeamSelect(TeamType::MARKETING),
                self::createUserSelect(TeamType::MARKETING),

                self::createTeamSelect(TeamType::CSKH),
                self::createUserSelect(TeamType::CSKH),
            ]);
    }

    protected static function getStatusSection(): Section
    {
        return Section::make(__('filament.product.status'))
    ...
    /**
     * Tạo Select field cho việc chọn Đội (Team).
     * @param TeamType $teamType
     * @return Select
     */
    protected static function createTeamSelect(TeamType $teamType): Select
    {
        $name = strtolower($teamType->name) . '_team_id';

        // Map team type to correct translation key
        $labelKey = match ($teamType) {
            TeamType::SALE => 'sale_team',
            TeamType::MARKETING => 'marketing_team',
            TeamType::CSKH => 'cskh_team',
            default => strtolower($teamType->name) . '_team',
        };

        $label = __("filament.product.{$labelKey}");
    ...
    /**
     * Tạo Select field cho việc chọn Nhân viên (Users) dựa trên Team được chọn.
     * @param TeamType $teamType
     * @return Select
     */
    protected static function createUserSelect(TeamType $teamType): Select
    {
        $relationshipName = match ($teamType) {
            TeamType::SALE => 'saleUsers',
            TeamType::MARKETING => 'marketingUsers',
            TeamType::CSKH => 'cskhUsers',
            default => strtolower($teamType->name) . 'Users',
        };

        // Map team type to correct translation key
        $labelKey = match ($teamType) {
            TeamType::SALE => 'sale_staff',
            TeamType::MARKETING => 'marketing_staff',
            TeamType::CSKH => 'cskh_staff',
            default => strtolower($teamType->name) . '_staff',
        };

        $teamFieldName = strtolower($teamType->name) . '_team_id';
        $label = __("filament.product.{$labelKey}");
        ...
        return Select::make($relationshipName)
            ...
            ->relationship(
                ...
                titleAttribute: 'name',
                modifyQueryUsing: function ($query, Get $get) use ($teamType, $teamFieldName) {
                    if ($teamId = $get($teamFieldName)) {
                        $query->where('team_id', $teamId)->where('disable', false);
                    } else {
                        $query->whereHas(
                            'teams',
                            fn($q) => $q->where('type', $teamType->value)
                        );
                    }
                }
            )
            ...
            ->searchable()
            ->preload()
            ->disabled(fn(Get $get) => !$get($teamFieldName))
