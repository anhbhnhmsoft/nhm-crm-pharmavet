...
class CustomerOperationsTable
{
    ...
    protected static function getOrderFinanceService(): OrderFinanceService
    {
    ...
    {
        return self::getOrderFinanceService()->getMaxAllowedDeposit(
            self::getOrderFinanceInputs($get, 0)
        );
    }

    protected static function getMaxAllowedCodSupport(Get $get): float
    {
        return self::getOrderFinanceService()->getMaxAllowedCodSupport(
            self::getOrderFinanceInputs($get)
        );
    }
    ...
    protected static function validateFinalizeOrderData(array $data, $record): void
    {
        ...
        $messages = array_merge(
            $messages,
            self::getOrderFinanceService()->getCollectValidationMessages($financeInputs)
        );
                    ...
                    Action::make('finalize_order')
                                        ...
                                        function (string $attribute, $value, $fail) use ($record) {
                                            ...
                                            Grid::make(3)->schema([
                                                ...
                                                TextInput::make('cod_support_amount')
                                                    ...
                                                    ->numeric()
                                                    ->minValue(0)
                                                    ->maxValue(fn(Get $get): float => self::getMaxAllowedCodSupport($get))
                                                    ->default(0)
                                                    ->extraInputAttributes(self::numericInputAttributes())
                                                    ->live(onBlur: true)
                                                    ->afterStateUpdated(fn(Get $get, Set $set) => self::recalculateOrderSummary($set, $get))
                                                    ->validationMessages([
                                                        'numeric' => __('common.error.numeric'),
                                                        'min' => __('common.error.min_value', ['min' => 0]),
                                                        'max' => __('telesale.messages.collect_adjustments_exceed_total'),
                                                    ]),
                        ...
                        ->action(function (array $data, $record, OrderService $orderService, $livewire) {
                            ...
                            if ($result->isSuccess()) {
                                ...
                                if ($result->getMessage() === __('telesale.messages.collect_adjustments_exceed_total')) {
                                    throw ValidationException::withMessages([
                                        'cod_support_amount' => $result->getMessage(),
                                    ]);
                                }
    ...
    {
        $financeService = self::getOrderFinanceService();
