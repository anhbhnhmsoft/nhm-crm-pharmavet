                    ...
                    <template x-if="reportData.sales">
                                    ...
                                    <table class="w-full text-left border-collapse">
                                        <thead>
                                            <tr class="bg-gray-50/50 dark:bg-gray-800/50">
                                                <th class="p-3 border-b border-gray-200 dark:border-gray-700 font-bold text-gray-900 dark:text-white">{{ __('accounting.report.source_name') }}</th>
                                                <th class="p-3 border-b border-gray-200 dark:border-gray-700 font-bold text-center text-gray-900 dark:text-white">{{ __('accounting.report.total_orders') }}</th>
                                                <th class="p-3 border-b border-gray-200 dark:border-gray-700 font-bold text-center text-green-600">{{ __('accounting.report.success') }}</th>
                                                <th class="p-3 border-b border-gray-200 dark:border-gray-700 font-bold text-center text-red-600">{{ __('accounting.report.returned') }}</th>
                                                <th class="p-3 border-b border-gray-200 dark:border-gray-700 font-bold text-center text-blue-600">{{ __('accounting.report.delivering') }}</th>
                                            </tr>
                                        </thead>
                                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                            <template x-for="item in reportData.marketing" :key="item.source">
                                                <tr class="hover:bg-gray-50/80 dark:hover:bg-gray-800/30 transition-colors">
                                                    <td class="p-3 font-semibold text-gray-700 dark:text-gray-300" x-text="item.source"></td>
                                                    <td class="p-3 text-center font-black text-gray-900 dark:text-white" x-text="item.total_count"></td>
                                                    <td class="p-3">
                                                        <div class="text-center text-green-700 dark:text-green-400 font-bold text-lg" x-text="item.success.count"></div>
                                                        <div class="text-[10px] text-center text-gray-400 font-bold uppercase tracking-tighter" x-text="new Intl.NumberFormat('vi-VN').format(item.success.cod) + ' COD'"></div>
                                                        <div class="flex justify-center mt-1">
                                                            <span class="px-2 py-0.5 rounded-full text-[10px] font-black bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 border border-green-200 dark:border-green-800" x-text="item.success.rate + '%'"></span>
                                                        </div>
                                                    </td>
                                                    <td class="p-3">
                                                        <div class="text-center text-red-700 dark:text-red-400 font-bold text-lg" x-text="item.returned.count"></div>
                                                        <div class="text-[10px] text-center text-gray-400 font-bold uppercase tracking-tighter" x-text="new Intl.NumberFormat('vi-VN').format(item.returned.cod) + ' COD'"></div>
                                                        <div class="flex justify-center mt-1">
                                                            <span class="px-2 py-0.5 rounded-full text-[10px] font-black bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 border border-red-200 dark:border-red-800" x-text="item.returned.rate + '%'"></span>
                                                        </div>
                                                    </td>
                                                    <td class="p-3">
                                                        <div class="text-center text-blue-700 dark:text-blue-400 font-bold text-lg" x-text="item.delivering.count"></div>
                                                        <div class="text-[10px] text-center text-gray-400 font-bold uppercase tracking-tighter" x-text="new Intl.NumberFormat('vi-VN').format(item.delivering.cod) + ' COD'"></div>
                                                        <div class="flex justify-center mt-1">
                                                            <span class="px-2 py-0.5 rounded-full text-[10px] font-black bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 border border-blue-200 dark:border-blue-800" x-text="item.delivering.rate + '%'"></span>
                                                        </div>
                                                    </td>
                                                </tr>
                                            </template>
                        ...
                        <x-filament::section>
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8"
                                x-data="{ categories: {{ json_encode($expenseCategories) }} }">
                                <!-- Summary -->
                                <div
                                ...
                                <div class="lg:col-span-8 space-y-4">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-4">
                                        <template
                                            x-for="(amount, categoryId) in reportData.business.expense?.by_category"
                                            :key="categoryId">
                                            <div class="group">
